...
bd_host = '91.227.16.16'
bd_login = 'h162801_anarchyfran'
bd_pass = 'K8q8R6i6N8'
bd_base = 'h162801_anarchy'
...
def start_message(message):
	if message.chat.type == 'private':
		...
		row = q.fetchall()
		if len(row) == 0:
			text = '<b>❗️ Перед началом использования сервисом пожалуйста ознакомьтесь с условиями и правилами сервиса.</b>'
			photo = open('/root/smm/photo.jpg', 'rb')
			bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
			...
			keyboard.add(types.InlineKeyboardButton(text='↗️ Ознакомиться ',url=f'https://telegra.ph/Pravila--Usloviya-04-23'))
			keyboard.add(types.InlineKeyboardButton(text='✔️ Я ознакомился и согласен с правилами сервиса !',callback_data=f'меню '))
			bot.send_message(message.chat.id,f'''<b>❗️ Перед началом использования сервисом пожалуйста ознакомьтесь с условиями и правилами сервиса.</b>''',parse_mode='HTML',reply_markup=keyboards.main)
		else:
			text = '👑 Добро пожаловать!'
			photo = open('/root/smm/photo.jpg', 'rb')
			bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboards.main)
		...
		if str(message.text[7:]) == str(f'sendall'):
			...
			if  message.chat.id == bot_admin:
				...
				text = '✍️<b>Напишите текст для рассылки:</b>'
				photo = open('/root/smm/photo.jpg', 'rb')
				msg = bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboards.main)
...
def send_text(message):
	...
	if message.text == '💳 Профиль':
		users_zadanie_del(message.chat.id)
...
<b>└Баланс:</b> <code>{row[6]} RUB</code>'''
		...
		keyboard.add(types.InlineKeyboardButton(text=f'''⬅️ Назад''',callback_data='/start'))
		text = f'''⚡️ Если у Вас есть вопрос по работе бота, нажмите на кнопку «Задать вопрос».

При проблеме с заказом, нажмите на кнопку «Проверка заказа» и пройдите по инструкции.'''
		photo = open('/root/smm/photo.jpg', 'rb')
		bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
		...
		text = f'''✍️<b>Напишите ID заказа (ID можно узнать во вкладке «Мои заказы»):</b>'''
		photo = open('/root/smm/photo.jpg', 'rb')
		msg = bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
		bot.register_next_step_handler(msg, statuszakaz, msg.message_id)

	elif message.text == '⚡️ Продвижение':
		users_zadanie_del(message.chat.id)
		users_zadanie_del(message.chat.id)
		bot.delete_message(chat_id=message.chat.id,message_id=message.message_id)
		connection = connect(host=bd_host,user=bd_login,password=bd_pass,database=bd_base)
		...
		while btns != []:
			try:
				...
				keyboard.add(btns[0])
				del btns[0]
		status = is_subscribed(message.chat.id)
		if status == True:
			connection.close()
			keyboard.add(types.InlineKeyboardButton(text='⬅️ Назад к меню', callback_data=f'/start'))
			text = f'''<b>📈 Что будем продвигать ?</b>'''
			photo = open('/root/smm/photo.jpg', 'rb')
			bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
		else:
			q.execute(f'SELECT chanel_id FROM chanel WHERE bot = "{bot_name}"')
			...
			bot.send_message(message.chat.id, f'''<b>❗️ Для использования бота необходимо вступить в наш канал:</b>
...
def supportadd(message,msdids):
	if message.chat.id in users_zadanie:
		...
		try:
			...
			connection.commit()
			connection.close()
			text = f'''✔️ Сообщение успешно отправлено в поддержку и в скорем времени будет рассмотрено.'''
			photo = open('/root/smm/photo.jpg', 'rb')
			bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
		except:
			...
			if row != None:
				...
				if ostatok_cupon >= 1:
					if chat_id in user_list:
						text = f'''🙁 Промокод был недействительным или мы не одобрили его использование.'''
						photo = open('/root/smm/photo.jpg', 'rb')
						bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
					else:
						...
						connection.commit()
						connection.close()
						text = f'''✔️ Баланс пополнен на {summa_cupon} RUB'''
						photo = open('/root/smm/photo.jpg', 'rb')
						bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
				else:
					connection.close()
...
def btc_oplata_2(message):
		try:
			connection = connect(host=bd_host,user=bd_login,password=bd_pass,database=bd_base)
			q = connection.cursor()
			sign = hashlib.md5(f"15824:{message.text}:[$^IJO_wH2z?(S0:RUB:Пополнение счета".encode()).digest().hex()
			#url = requests.post(url='https://pay.freekassa.ru', data={'m': 15824, 'oa': {message.text}, 'currency': 'RUB', 'o': 'оплата товара', 's': {sign}})
			url = f'https://pay.freekassa.ru/?m=15824&oa={message.text}&o=Пополнение счета&s={sign}&currency=RUB'
			q.execute("INSERT INTO pay_ok (user,id_pay,summa) VALUES ('%s', '%s', '%s')"%(message.chat.id,message.text, sign))
			connection.commit()
			keyboard = types.InlineKeyboardMarkup()
			...
			keyboard.add(types.InlineKeyboardButton(text=f'''⬅️ Назад''',callback_data=f'профиль '))
			text = f'''❗️ Для пополнения баланса, перейдите по ссылке  ниже и оплатите счет удобным способом !

💡 После оплаты, вы получите уведомление о зачисление средств на баланс.'''
			photo = open('/root/smm/photo.jpg', 'rb')
			bot.send_photo(message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
		except Exception as e:
			print(e)
			text = f'''❗️ Минимальная сумма пополнения 10 RUB'''
			photo = open('/root/smm/photo.jpg', 'rb')
...
def btc_oplata_1(message):
		try:
			if int(message.text) >= int(1):
				...
				keyboard = types.InlineKeyboardMarkup()
				keyboard.add(types.InlineKeyboardButton(text=f'''↗️ Перейти к оплате ''',url=url))
				keyboard.add(types.InlineKeyboardButton(text=f'''🦋Проверить платеж''',callback_data=f'1'))
				keyboard.add(types.InlineKeyboardButton(text=f'''⬅️ Назад''',callback_data=f'профиль '))
...
💡 После оплаты, вы получите уведомление о зачисление средств на баланс.''', reply_markup=keyboard)
			...
			bot.send_message(message.chat.id, f'❗️ Ошибка',parse_mode='HTML')
...
def podcategors(call):
	...
	if a[0] == 'меню':
		...
		try:
			...
			text = 'Привет Используй меню, чтобы со мной общаться 💬'
			photo = open('/root/smm/photo.png', 'rb')
	...
	if a[0] == 'check_opl2':
		...
		q.execute(f"SELECT id_pay FROM pay_ok where user = '{call.message.chat.id}' ORDER BY id DESC LIMIT 1")
		money = q.fetchone()[0]
		sign = hashlib.md5(f"15824:{money}:[$^IJO_wH2z?(S0:RUB:Пополнение счета".encode()).digest().hex()
		#url = requests.post(url='https://pay.freekassa.ru', data={'m': 15824, 'oa': {message.text}, 'currency': 'RUB', 'o': 'оплата товара', 's': {sign}})
		url = f'https://pay.freekassa.ru/?m=15824&oa={money}&o=Пополнение счета&s={sign}&currency=RUB'

		try:
			url2 = requests.post(url='https://pay.freekassa.ru/', data={'m': 15824, 'oa': money, 'o': 'Пополнение счета', 's': zalupa,})
...
<b>└Ссылка:</b> <a href="{invite_link}">{invite_link}</a>''',parse_mode='HTML', reply_markup=keyboard,disable_web_page_preview = True)
		...
		text = f'<b>📈 Какая услуга вас интересует ?</b>'
		photo = open('/root/smm/photo.jpg', 'rb')
		bot.send_photo(call.message.chat.id,  photo, caption=text,parse_mode='HTML', reply_markup=keyboard)
...
<b>└Если вы пригласите человека который пополнит баланс в боте, вам дадут 5% от пополнения на ваш баланс!</b>''',parse_mode='HTML',reply_markup=keyboard, disable_web_page_preview=True)	
		...
		try:
			...
			if int(confzakaz[6]) >= int(prace):
				tomorrow = datetime.now()
				data = tomorrow.strftime('%d/%m/%Y')
				apinakrutka  = '9fbcf99969df9c26d86ea734b137b126'
				idnakrutka  = rowsss[6]
				link  = confzakaz[13]
				colvo  = confzakaz[12]
