...
bd_login = 'h159987_posting'
bd_pass = 'jopa1337123'
bd_base = 'h159987_bbb1'
...
bot_token = '1626162520:AAHzbRmTgJyJlGqvBXLiaZ_N-T3YbQcLndA'
...
def start_message(message):
	if message.chat.type == 'private':
		...
		if len(row) == 0:
			...
			bot.send_message(message.chat.id, '👑 Добро пожаловать!', parse_mode='HTML', reply_markup=keyboards.main)
		...
		if str(message.text[7:]) == str(f'sendall'):
			...
			if  message.chat.id == bot_admin:
				...
				msg = bot.send_message(message.chat.id,  '✍️<b>Напишите текст для рассылки:</b>', parse_mode='HTML', reply_markup=keyboards.main)
...
def send_text(message):
	...
	if message.text == '💳 Профиль':
		...
		bot.send_message(message.chat.id,  f'''<b>👤 Профиль:
├Id:</b> <code>{message.chat.id}</code>
<b>└Баланс:</b> <code>{row[6]} RUB</code>''', parse_mode='HTML', reply_markup=keyboard)
		...
		bot.send_message(message.chat.id,  f'''⚡️ Если у Вас есть вопрос по работе бота, нажмите на кнопку «Задать вопрос».
...
При проблеме с заказом, нажмите на кнопку «Проверка заказа» и пройдите по инструкции.''', parse_mode='HTML', reply_markup=keyboard)
		...
		msg = bot.send_message(message.chat.id,  f'''✍️<b>Напишите ID заказа (ID можно узнать во вкладке «Мои заказы»):</b>''', parse_mode='HTML', reply_markup=keyboard)
		...
		if status == True:
			...
			bot.send_message(message.chat.id,  f'''<b>📈 Что будем продвигать ?</b>''', parse_mode='HTML', reply_markup=keyboard)
		else:
			q.execute(f'SELECT chanel_id FROM chanel WHERE bot = "{bot_name}"')
			row = q.fetchone()
			connection.close()
...
def supportadd(message,msdids):
	if message.chat.id in users_zadanie:
		...
		try:
			...
			connection.commit()
			connection.close()
			bot.send_message(message.chat.id,  f'''✔️ Сообщение успешно отправлено в поддержку и в скорем времени будет рассмотрено.''', parse_mode='HTML', reply_markup=keyboard)
		except:
...
def btc_oplata_2(message):
		try:
			...
			keyboard.add(types.InlineKeyboardButton(text=f'''⬅️ Назад''',callback_data=f'профиль '))
			bot.send_message(message.chat.id,  f'''❗️ Для пополнения баланса, перейдите по ссылке  ниже и оплатите счет удобным способом !

💡 После оплаты, вы получите уведомление о зачисление средств на баланс.''', parse_mode='HTML', reply_markup=keyboard)
...
def podcategors(call):
	...
	if a[0] == 'меню':
		...
		try:
			...
			bot.send_message(call.message.chat.id,  'Привет Используй меню, чтобы со мной общаться 💬', parse_mode='HTML', reply_markup=keyboards.main)
		except Exception as e:
			pass
